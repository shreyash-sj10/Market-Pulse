...
export default function AdaptiveProfileCard({ profile, loading }) {
  if (loading) return <div className="h-64 bg-slate-900 animate-pulse rounded-[2.5rem]" />;
  if (!profile) return null;

  const { sensitivityLevel, dominantPatterns, riskBias, rulesApplied } = profile;

  return (
    <motion.div 
      initial={{ opacity: 0, x: 20 }}
      animate={{ opacity: 1, x: 0 }}
      className="p-10 bg-slate-930 text-white rounded-[2.5rem] border border-white/10 shadow-2xl relative overflow-hidden"
    >
       {/* Background Glow */}
       ...
       <div className="flex items-center gap-4 mb-10">
          ...
          <div>
             <h3 className="text-xl font-black tracking-tight">Trading Persona</h3>
