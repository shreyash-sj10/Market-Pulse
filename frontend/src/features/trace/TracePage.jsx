...
const TracePage = () => {
  ...
  const { traces, selectedTraceId, selectedTrace, listLoading, detailLoading, selectTrace } = useTrace();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          {listLoading ? (
            ...
            safeArray(traces).map(t => (
              <MotionDiv
                key={t.id}
                whileHover={{ x: 5 }}
                onClick={() => selectTrace(t.id)}
                className={`p-6 rounded-[2rem] border transition-all cursor-pointer ${selectedTraceId === t.id ? 'bg-slate-900 text-white border-slate-900 shadow-xl' : 'bg-white text-slate-900 border-slate-100 hover:border-indigo-600'}`}
              >
                <div className="flex items-center justify-between mb-3">
                   <div className="flex items-center gap-2">
                       <Workflow size={14} className={selectedTraceId === t.id ? "text-indigo-400" : "text-indigo-600"} />
                       <span className="text-[10px] font-black uppercase tracking-widest">{t.type === 'ANALYSIS' ? 'DECISION' : t.type}</span>
                   </div>
                   <span className={`text-[8px] font-bold uppercase opacity-50 ${selectedTraceId === t.id ? 'text-white' : 'text-slate-400'}`}>
                      {new Date(t.timestamp).toLocaleTimeString()}
                   </span>
                </div>
                <div className="flex items-center justify-between">
                   <div>
                      <span className="text-xs font-bold tracking-tight opacity-80 block">Audit ID: {String(t.id).slice(-8).toUpperCase()}</span>
                      {t.verdict && <span className={`text-[8px] font-black uppercase tracking-widest mt-1 block ${t.verdict === 'BUY' ? 'text-emerald-500' : 'text-rose-500'}`}>{t.verdict} VERDICT</span>}
                      <span className="text-[9px] font-medium opacity-70 block mt-2">{t.summary}</span>
                   </div>
        ...
        <div className="lg:col-span-8">
           <AnimatePresence mode="wait">
             {!selectedTraceId ? (
                ...
                <MotionDiv key={selectedTrace._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                         ...
                         <div className="space-y-3">
                            ...
                            <div className="flex flex-col gap-1 mt-2">
                               ...
                               <div className="flex flex-wrap gap-1">
                                  {safeArray(selectedTrace.stages.constraint_engine.rules_applied).map(r => (
