...
const SystemDashboard = () => {
  const { data: summaryResponse } = useQuery({ queryKey: queryKeys.portfolioSummary(), queryFn: getPortfolioSummary });
  const { data: positionsResponse } = useQuery({ queryKey: queryKeys.positions(), queryFn: getPositions });
  const { data: tradesResponse } = useQuery({ queryKey: queryKeys.tradeHistory(1, 1), queryFn: () => getTradeHistory(1, 1) });
  const { data: profileResponse } = useQuery({ queryKey: queryKeys.profile(), queryFn: getUserProfile });
          ...
          <div className="bg-white rounded-[3rem] p-12 border border-slate-100 shadow-sm">
            ...
            <div className="space-y-8">
              {[
                ...
                { label: "Cache Layer", detail: "Redis / degraded fallback", status: "NON-BLOCKING", latency: "env-bound" },
                ...
                <div key={i} className="flex items-center justify-between py-6 border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition-colors px-4 rounded-2xl">
                  ...
                  <div className="flex items-center gap-8 text-right">
                    <div>
                      <span className="block text-[8px] font-bold text-slate-400 uppercase mb-1">Status</span>
                      <span className="text-[9px] font-black text-emerald-600 tracking-widest">{item.status}</span>
                    </div>
                    <div>
                      <span className="block text-[8px] font-bold text-slate-400 uppercase mb-1">Latency</span>
            ...
            <div>
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-black">{profile?.totalTrades || trades.length}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs font-bold text-slate-400">Active Positions</span>
                  <span className="text-xl font-black">{positions.length}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs font-bold text-slate-400">Portfolio Value</span>
                  <span className="text-xl font-black text-indigo-400">{summary?.totalValuePaise || 0}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs font-bold text-slate-400">Skill Score</span>
                  <span className="text-xl font-black text-emerald-400">{profile?.skillScore || 0}</span>
                </div>
              </div>
            </div>
